...
<div class="wrap woocommerce">
	<nav class="nav-tab-wrapper woo-nav-tab-wrapper">
		...
		foreach ( $reports as $key => $report_group ) {
			echo '<a href="' . admin_url( 'admin.php?page=sos-leaderboard&tab=' . urlencode( $key ) ) . '" class="nav-tab ';
			if ( $current_tab == $key ) {
				echo 'nav-tab-active';
			}
	...
	<?php if ( sizeof( $reports[ $current_tab ]['reports'] ) > 1 ) {
		...
		<ul class="subsubsub">
			...
			foreach ( $reports[ $current_tab ]['reports'] as $key => $report ) {
				...
				$link = '<a href="admin.php?page=sos-leaderboard&tab=' . urlencode( $current_tab ) . '&amp;report=' . urlencode( $key ) . '" class="';
	...
	?>
		...
		<div class="inside">
			<table class="wp-list-table widefat fixed striped posts">
				...
				<tbody>
					<?php $this->get_order_report_data(); ?>
				</tbody>
			</table>
