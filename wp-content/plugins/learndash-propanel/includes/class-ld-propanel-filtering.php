		...
		function show_filters() {
			if ( !empty( $this->registered_filters ) ) {
				foreach( $this->registered_filters as $filter_key => $filter_set ) {
					?><div class="select2-selection filter-selection filter-section-<?php echo $filter_key ?>"  data-filter-key="<?php echo $filter_key ?>" style="display: inline-block;"><?php echo $filter_set['instance']->filter_display(); ?></div><?php
				}
			}
		}
			...
			if ( ( ! empty( $user_ids ) ) && ( !empty( $subject ) ) && ( !empty( $message ) ) ) {
				...
				if ( $result ) {
					wp_send_json_success( 
						array(
							'message' => sprintf( __('Email sent to %d destinations', 'ld_propanel' ), count( $user_ids ) ),
					...
					wp_send_json_error( 
						array( 
							'message' => sprintf( esc_html__( "We could not send the email successfully. Please try again or check with your hosting provider.
Error: %s", 'ld_propanel' ), $error_string )
						) 
					);
				}
			} else {
