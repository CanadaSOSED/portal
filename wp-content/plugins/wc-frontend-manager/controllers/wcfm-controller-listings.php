...
class WCFM_Listings_Controller {
	...
	public function processing() {
		...
		$wcfm_listings_count = wp_count_posts('job_listing');
		$listing_count = ( $wcfm_listings_count->publish + $wcfm_listings_count->pending + $wcfm_listings_count->draft );
		...
		if(!empty($wcfm_listings_array)) {
			...
			foreach($wcfm_listings_array as $wcfm_listings_single) {
				...
				// Listing
				$wcfm_listings_json_arr[$index][] =  '<a href="' . add_query_arg( array( 'action' => 'edit', 'job_id' => $wcfm_listings_single->ID ), $jobs_dashboard_url ) . '" class="wcfm_listing_title">' . $wcfm_listings_single->post_title . '</a>';
