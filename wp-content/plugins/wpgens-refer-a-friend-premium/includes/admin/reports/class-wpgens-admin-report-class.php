 ...
 * @version     2.1.0
 */
class WC_Report_Gens_Raf extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		// Filtrira samo ordere koji imaju _raf_meta , treba izbaciti _raf_meta i naci drugi nacin...
		...
		// remove false orders
		foreach ($this->report_data->order_counts as $key => $order) {
			$meta = get_post_meta($order->id,'_raf_meta',true);
			if (!empty($meta)){
		...
		foreach ($this->report_data->coupons as $key => $order) {
			$meta = get_post_meta($order->id,'_raf_meta',true);
			if (!empty($meta)){
				if($meta['publish'] == "false") {
		...
		foreach ($this->report_data->order_items as $key => $order) {
			$meta = get_post_meta($order->id,'_raf_meta',true);
			if (!empty($meta)){
				if($meta['publish'] == "false") {
