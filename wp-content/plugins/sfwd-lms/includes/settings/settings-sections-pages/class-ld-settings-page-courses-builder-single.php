	...
	class LearnDash_Settings_Page_Course_Builder_Single extends LearnDash_Settings_Page {
		...
		public function settings_page_after_title( $settings_screen_id = '' ) {
			if ( $this->settings_screen_id == $settings_screen_id ) {
				if ( ( isset( $_GET['course_id'] ) ) && ( !empty( $_GET['course_id'] ) ) ) {
					...
					if ( ( $course_post ) && ( is_a( $course_post, 'WP_Post' ) ) && ( 'sfwd-courses' === $course_post->post_type )  ) {
						?>
		...
		public function save_cb_metabox() {
			if ( ( isset( $_GET['course_id'] ) ) && ( ! empty( $_GET['course_id'] ) ) ) {
				...
				$course_post = get_post( $course_id );
				if ( ( $course_post ) && ( is_a( $course_post, 'WP_Post' ) ) && ( 'sfwd-courses' == $course_post->post_type )  ) {
					$this->update_success = $this->cb->save_course_builder( $course_id, $course_post, true );
				}
			}
			return;
