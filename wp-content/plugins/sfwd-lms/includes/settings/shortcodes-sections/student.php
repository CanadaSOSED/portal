...
if ( ( class_exists( 'LearnDash_Shortcodes_Section' ) ) && ( !class_exists( 'LearnDash_Shortcodes_Section_student' ) ) ) {
	class LearnDash_Shortcodes_Section_student extends LearnDash_Shortcodes_Section {
		...
		function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_description	=	sprintf( wp_kses_post( _x( 'This shortcode shows the content if the user is enrolled in the %s. The shortcode can be used on <strong>any</strong> page or widget area.', 'placeholders: course', 'learndash' ) ), LearnDash_Custom_Label::label_to_lower( 'course' ) );
