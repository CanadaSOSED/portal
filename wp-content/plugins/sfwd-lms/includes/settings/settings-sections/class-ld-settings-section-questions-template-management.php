<?php
/**
 * LearnDash Settings Section Question Template Mangement.
 *
 * @package LearnDash
 * @subpackage Settings
 */

if ( ( class_exists( 'LearnDash_Settings_Section' ) ) && ( ! class_exists( 'LearnDash_Settings_Questions_Template_Management' ) ) ) {
	/**
	 * Class to create the settings section.
	 */
	class LearnDash_Settings_Questions_Template_Management extends LearnDash_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'learndash_settings_questions_template_management';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'learndash_settings_questions_template_management';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'questions_template_management';

			// Section label/header.
			$this->settings_section_label = sprintf(
				// translators: placeholder: Quiz.
				esc_html_x( '%s Template Management', 'placeholder: Question', 'learndash' ), LearnDash_Custom_Label::get_label( 'question' )
			);
		...
		public function load_settings_fields() {
			$this->setting_option_fields = array(
				'question_template' => array(
					...
					'help_text' => sprintf(
						// translators: placeholder: Question.
						esc_html_x( 'Manage %s templates. Select a template then update the title or delete.', 'placeholder: Question', 'learndash' ), LearnDash_Custom_Label::get_label( 'question' )
					),
					'value'     => '',
						...
						if ( 'update' === $_POST['field_action'] ) {
							if ( ( isset( $_POST['field_value'] ) ) && ( ! empty( $_POST['field_value'] ) )
							  && ( isset( $_POST['field_text'] ) ) && ( ! empty( $_POST['field_text'] ) ) ) {
								...
								$template = $templateMapper->fetchById( $template_id );
								if ( ( $template ) && ( is_a( $template, 'WpProQuiz_Model_Template' ) ) ) {
									...
									if ( $template_current_name != $template_new_name ) {
										$update_ret = $templateMapper->updateName( $template_id, $template_new_name );
										if ( $update_ret ) {
							...
							if ( ( isset( $_POST['field_value'] ) ) && ( ! empty( $_POST['field_value'] ) ) ) {
								...
								$template = $templateMapper->fetchById( $template_id );
								if ( ( $template ) && ( is_a( $template, 'WpProQuiz_Model_Template' ) ) ) {
									$update_ret = $templateMapper->delete( $template_id );
									if ( $update_ret ) {
										$reply_data['status'] = true;
...
add_action( 'learndash_settings_sections_init', function() {
	LearnDash_Settings_Questions_Template_Management::add_section_instance();
} );
