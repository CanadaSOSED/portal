...
if ( ( class_exists( 'LearnDash_Settings_Section' ) ) && ( ! class_exists( 'LearnDash_Settings_Section_PayPal' ) ) ) {
	...
	class LearnDash_Settings_Section_PayPal extends LearnDash_Settings_Section {
		...
		protected function __construct() {
			...
			// Check if curl.cainfo is set.
			$this->maybe_show_missing_ca_certificates_notice();
			...
			$this->setting_option_fields = array(
				...
				'paypal_currency' => array(
					...
					'help_text' => sprintf(
						...
						esc_html_x( 'Enter the currency code for transactions. See PayPal %s Documentation', 'placeholder: URL to PayPal Currency Codes', 'learndash' ), '<a href="https://developer.paypal.com/docs/classic/api/currency_codes/" target="_blank">' . __( 'Currency Codes', 'learndash' ) . '</a>' ),
		...
		/**
		 * Check the path for the CA certificates and show a message if it is not set.
		 */
		public function maybe_show_missing_ca_certificates_notice() {
			$certificates = ini_get('curl.cainfo');

			if ( ( isset( $_GET['page'] ) ) && ( $_GET['page'] == $this->settings_page_id ) && ! $certificates ) {
				add_action('admin_notices', function() {
					$notice = __( 'Path to the CA certificates not set. Please add it to curl.cainfo in the php.ini file. Otherwise, PayPal may not work.', 'learndash' );
					printf( '<div class="notice notice-error"><p>%1$s</p></div>', esc_html( $notice ) );
				});
			}
		}
