	...
	<form action="admin.php?page=ldAdvQuiz&module=question&action=addEdit&quiz_id=<?php echo $this->quiz->getId(); ?>&questionId=<?php echo $this->question->getId(); ?>&post_id=<?php echo @$_GET['post_id']; ?>" method="POST">
		...
		<div id="poststuff">
			<div class="postbox">
				...
				<div class="inside">
					<p class="description">
						<?php echo sprintf( esc_html_x('The title is used for overview, it is not visible in %s. If you leave the title field empty, a title will be generated.', 'placeholders: quiz', 'learndash'), LearnDash_Custom_Label::label_to_lower( 'quiz' ) ); ?>
					</p>
			...
			<div class="postbox" style="<?php echo $this->quiz->isHideAnswerMessageBox() ? '' : 'display: none;'; ?>">
				...
				<div class="inside">
					<?php echo sprintf( esc_html_x('Deactivated in %s settings.', 'Deactivated in quiz settings.', 'learndash'), LearnDash_Custom_Label::label_to_lower( 'quiz' ) ); ?>
				</div>
			</div>
	...
	public function essayChoice( $data ) {
		...
		if ( is_a( $data, 'WpProQuiz_Model_AnswerTypes' ) ) {
			?>
				...
				<p class="description" style="margin-top: 10px">
					<?php echo sprintf( esc_html_x( 'This is a question that can be graded and potentially prevent a user from progressing to the next step of the %s.', 'placeholders: course', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'course' ) ) ?><br />
					...
					<?php echo sprintf( esc_html_x( 'How should the answer to this question be marked and graded upon %s submission?', 'placeholders: quiz', 'learndash' ), LearnDash_Custom_Label::label_to_lower( 'quiz' ) ); ?><br />
				</p>
