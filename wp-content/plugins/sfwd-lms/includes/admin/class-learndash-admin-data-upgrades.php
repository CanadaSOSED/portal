		...
		public function show_upgrade_admin_notice() {
			if ( true !== self::$admin_notice_shown ) {
				...
				$admin_notice_message = sprintf(
					...
					esc_html_x( 'LearnDash Notice: Please perform a %s. This is a required step to ensure accurate reporting.', 'placeholder: link to LearnDash Data Upgrade admin page', 'learndash' ), '<a href="' . admin_url( 'admin.php?page=learndash_data_upgrades' ) . '">' . esc_html__( 'LearnDash Data Upgrade', 'learndash' ) . '</a>'
				);
				?>
			...
			if ( ! empty( $data_settings ) ) {
				...
				if ( ( $user ) && ( is_a( $user, 'WP_User' ) ) ) {
					$last_run_info = sprintf(
						...
						learndash_adjust_date_time_display( $data_settings['last_run'] ), $user->display_name
					);
				} 
			}
