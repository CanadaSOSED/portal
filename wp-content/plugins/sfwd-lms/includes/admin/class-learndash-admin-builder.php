...
if ( ! class_exists( 'Learndash_Admin_Builder' ) ) {
	...
	class Learndash_Admin_Builder {
		...
		public function builder_on_load() {
			...
			wp_register_script(
				'learndash-builder-script',
				LEARNDASH_LMS_PLUGIN_URL . 'assets/js/learndash-builder' . ( ( defined( 'LEARNDASH_SCRIPT_DEBUG' ) && ( LEARNDASH_SCRIPT_DEBUG === true ) ) ? '' : '.min' ) . '.js',
				array( 'jquery', 'jquery-ui-accordion', 'jquery-ui-dialog' ),
				LEARNDASH_SCRIPT_VERSION_TOKEN,
				true
			);
			$learndash_assets_loaded['scripts']['learndash-builder-script'] = __FUNCTION__;

			wp_enqueue_style(
				'learndash-builder-style',
				LEARNDASH_LMS_PLUGIN_URL . 'assets/css/learndash-builder' . ( ( defined( 'LEARNDASH_SCRIPT_DEBUG' ) && ( LEARNDASH_SCRIPT_DEBUG === true ) ) ? '' : '.min' ) . '.css',
				array(),
				LEARNDASH_SCRIPT_VERSION_TOKEN
			);
			wp_style_add_data( 'learndash-builder-style', 'rtl', 'replace' );

			$learndash_assets_loaded['styles']['learndash-builder-style'] = __FUNCTION__;
		}
		...
		public function builder_admin_footer() {
			...
			wp_localize_script( 'learndash-builder-script', 'learndash_builder_assets', $this->builder_assets );
			wp_enqueue_script( 'learndash-builder-script' );
		}
		...
		public function show_builder_box( $post ) {
			...
			wp_nonce_field( $this->builder_prefix . '_nonce_' . $this->builder_post_id, $this->builder_prefix . '_nonce' );
			?>
...
add_action( 'wp_ajax_learndash_builder_selector_pager', function() {
...
add_action( 'wp_ajax_learndash_builder_selector_search', function() {
	if ( ( isset( $_POST['builder_data'] ) ) && ( ! empty( $_POST['builder_data'] ) ) ) {
		if ( ( isset( $_POST['builder_data']['builder_class'] ) ) && ( ! empty( $_POST['builder_data']['builder_class'] ) ) ) {
			$builder_class = esc_attr( $_POST['builder_data']['builder_class'] );
			$builder_instance = $builder_class::add_instance();
...
add_action( 'wp_ajax_learndash_builder_selector_step_trash', function() {
	if ( ( isset( $_POST['builder_data'] ) ) && ( ! empty( $_POST['builder_data'] ) ) ) {
		if ( ( isset( $_POST['builder_data']['builder_class'] ) ) && ( ! empty( $_POST['builder_data']['builder_class'] ) ) ) {
			$builder_class = esc_attr( $_POST['builder_data']['builder_class'] );
			$builder_instance = $builder_class::add_instance();
...
add_action( 'wp_ajax_learndash_builder_selector_step_new', function() {
	if ( ( isset( $_POST['builder_data'] ) ) && ( ! empty( $_POST['builder_data'] ) ) ) {
		if ( ( isset( $_POST['builder_data']['builder_class'] ) ) && ( ! empty( $_POST['builder_data']['builder_class'] ) ) ) {
			$builder_class = esc_attr( $_POST['builder_data']['builder_class'] );
			$builder_instance = $builder_class::add_instance();
