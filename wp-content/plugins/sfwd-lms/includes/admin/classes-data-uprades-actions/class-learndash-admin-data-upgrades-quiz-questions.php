	...
	class Learndash_Admin_Data_Upgrades_Quiz_Questions extends Learndash_Admin_Data_Upgrades {
			...
			<tr id="learndash-data-upgrades-container-<?php echo esc_attr( $this->data_slug ); ?>" class="learndash-data-upgrades-container">
				...
				<td class="learndash-data-upgrades-status-container">
					...
					printf(
						...
						LearnDash_Custom_Label::label_to_lower( 'Quiz' ), LearnDash_Custom_Label::label_to_lower( 'questions' ), LearnDash_Custom_Label::get_label( 'Quiz' )
					);
					?>
					</p>
					...
					<?php
						...
						if ( ( true === $show_progess ) && ( ! isset( $_GET['quiz_id'] ) ) )  {
							...
							if ( ( isset( $data_settings['last_run'] ) ) && ( ! empty( $data_settings['last_run'] ) ) ) {
								...
								if ( ! empty( $process_quiz_id ) ) {
									?>
									<p id="learndash-data-upgrades-quiz-<?php echo esc_attr( $this->data_slug ); ?>" class="learndash-data-upgrades-quiz"><?php printf(
									...
									?>
									<p id="learndash-data-upgrades-mismatched-<?php echo esc_attr( $this->data_slug ); ?>" class="learndash-data-upgrades-mismatched"><input type="checkbox" name="learndash-data-upgrades-mismatched" value="1" checked="checked" /> <?php
									printf(
		...
		protected function query_items( $increment_paged = true ) {
			...
			if ( ( isset( $this->transient_data['quiz_id'] ) ) && ( ! empty( $this->transient_data['quiz_id'] ) ) ) {
				...
				if ( ( isset( $this->transient_data['total_count'] ) ) && ( ! empty( $this->transient_data['total_count'] ) ) ) {
					...
					error_log('sql_count_rows['. $sql_count_rows .']');
		...
		protected function build_progress_output( $data = array() ) {
			...
			$data['progress_label'] = sprintf(
				...
				esc_html_x( '%1$s: %2$d of %3$d %4$s', 'placeholders: progress status, result count, total count, Questions label', 'learndash' ), $progress_status, $data['result_count'], $data['total_count'], ( $data['total_count'] > 1) ? LearnDash_Custom_Label::get_label( 'questions' ) : LearnDash_Custom_Label::get_label( 'question' )
			);
