...
if ( ( !class_exists( 'LD_REST_Posts_Topics_Controller' ) ) && ( class_exists( 'LD_REST_Posts_Controller' ) ) ) {
	class LD_REST_Posts_Topics_Controller extends LD_REST_Posts_Controller {
		...
		function topic_mark_complete( $request ) {
			...
			if ( empty( $course_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Invalid Course ID.' ), array( 'status' => 404 ) );
			}
			
			if ( empty( $lesson_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Invalid Lesson ID.' ), array( 'status' => 404 ) );
			}
