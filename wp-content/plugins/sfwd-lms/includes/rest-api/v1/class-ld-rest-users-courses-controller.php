...
if ( ( !class_exists( 'LD_REST_Users_Courses_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	class LD_REST_Users_Courses_Controller_V1 extends LD_REST_Posts_Controller_V1 {
			...
			register_rest_route( 
				...
				array(
                    ...
                    array(
						...
						'args' => array(
							'course_ids' => array(
								'description' => esc_html__( 'Courses IDs to add to User.', 'learndash' ),
	    						'required' => true,
								'type' => 'array',
								'items' => array(
									'type' => 'integer'
					...
					array(
						...
						'args' => array(
							'course_ids' => array(
								'description' => esc_html__( 'Course IDs to remove from User.', 'learndash' ),
	    						'required' => true,
								'type' => 'array',
								'items' => array(
									'type' => 'integer'
		...
		function update_user_courses_permissions_check( $request ) {
			if ( learndash_is_admin_user( ) ) {
				return true;
			} else if ( get_current_user_id() == $request['id'] ) {
				return true;
		...
		function delete_user_courses_permissions_check( $request ) {
			if ( learndash_is_admin_user( ) ) {
				return true;
			} else if ( get_current_user_id() == $request['id'] ) {
				return true;
		...
		function update_user_courses( $request ) {
			$user_id = $request['id'];
			if ( empty( $user_id ) ) {
		...
		function delete_user_courses( $request ) {
			$user_id = $request['id'];
			if ( empty( $user_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Invalid User ID.', 'learndash' ) .' '. __CLASS__, array( 'status' => 404 ) );
			}
